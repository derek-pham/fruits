...
function Banana() {
    ...
    useEffect(() => {
        setPosition(prevPosition => prevPosition.set(-10 - 1.5, listNumber * 2.5 - 2.5, 0));
        if (listNumber == 1) {
            setItemScale(2);
        } else {
            setItemScale(1.5);
    ...
    return (
        <>
            <primitive ref={fruitRef} object={scene} scale={itemScale} position={[-10 - 1.5, listNumber - 2.5, 0]} />
        </>
    );
}
